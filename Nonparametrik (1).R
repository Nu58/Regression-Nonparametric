...
library(readxl)
library(readxl)
Data_RSTUDIO <- read_excel("C:/SKRIPSI/Pendukung/Data RSTUDIO.xlsx")
View(Data_RSTUDIO)
dat=as.data.frame(Data_RSTUDIO)
dat
...
gcv1knot<-function(x,y,b){
  ...
  print(list(knot=knott,gcv=gcv,gcv.minimum=optimum,knot.optimum=knot.optimum))
  return(hasil)
}
...
hitung<-function(x,y,knot,taraf.alpha){
  ...
  MSE=SSE/dbe
  MSR=SSR/dbr
  Se=sqrt(MSE*diag(babi)) #diag(pinv(t(matriksX)%*%matriksX)))
  Fhitung=MSR/MSE
...
spline.truncated.linier.multivariabel=function(x,y,b,taraf.alpha){
  ...
  kenormalan=ks.test(
    ...
    residual,"pnorm",mean=mean(residual),sd=sd(residual))
